...
caminho = r'C:\Users\Ânderson Fischoeder\Desktop\Tese\Modelos\ModeloAjustado\Repeticoes\Cristina'
cmh = os.path.join(caminho, 'P6') 
################################################Potreiro 20 Infestado############################################################# 
# load dataset
dataset1 = read_csv('e6_cristina.csv', header=0, index_col=0, delimiter=';')
...
# load dataset
dataset3 = read_csv('entrada_CN.csv', header=0, index_col=0, delimiter=';')
...
# load dataset
dataset5 = read_csv('entrada_TotalCristina.csv', header=0, index_col=0, delimiter=';')
values5 = dataset5.values
# integer encode direction
encoder = LabelEncoder()

dataset6 = read_csv('e6_cristina_Teste.csv', header=0, index_col=0, delimiter=';')
...
if not os.path.exists('M_TOTALCRISTINA.h5'):
	print('modelo inexistente... criando modelo...')
	# design network
	model = Sequential()
	#model.add(LSTM(30, input_shape=(train_X.shape[1], train_X.shape[2]),  kernel_initializer='normal', activation='tanh', return_sequences = True))#bom para 1 e 2
	...
	# save the network's architecture
	print('Salvando modelo...')
	#model.save('M_TOTALCRISTINA.h5')
	#------------------
else:
	#------------------
	# load the network's architecture
	model = load_model('M_TOTALCRISTINA.h5')
	history =  model
	#------------------
	print('Modelo encontrado e carregado')
...
pyplot.legend()
dpi = fig.get_dpi()
pyplot.savefig(cmh + '\Treinamentos_5.png', dpi=dpi*2)
pyplot.close()
...
pyplot.title('P6')
dpi = fig1.get_dpi()
pyplot.savefig(cmh + '\Boxplot_P_5.png', dpi=dpi*2)
pyplot.close()
...
pyplot.savefig(cmh + '\Dispersao_P_5.png', dpi=dpi*2)
pyplot.close()
...
pyplot.title('P6')
dpi = fig1.get_dpi()
pyplot.savefig(cmh + '\Boxplot_T_5.png', dpi=dpi*2)
pyplot.close()
#Gráfico de Dispersão
...
pyplot.ylabel('Real')
pyplot.xlabel('Predito')
pyplot.savefig(cmh + '\Dispersao_T_5.png', dpi=dpi*2)
pyplot.close()
...
pyplot.title('P6')
dpi = fig1.get_dpi()
pyplot.savefig(cmh + '\Boxplot_N_5.png', dpi=dpi*2)
pyplot.close()
#Gráfico de Dispersão
...
pyplot.ylabel('Real')
pyplot.xlabel('Predito')
pyplot.savefig(cmh + '\Dispersao_N_5.png', dpi=dpi*2)
pyplot.close()
