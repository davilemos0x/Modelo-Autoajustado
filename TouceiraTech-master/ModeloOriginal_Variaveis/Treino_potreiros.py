...
caminho = r'C:\Users\Ânderson Fischoeder\Desktop\Tese\Modelos\ModeloOriginal_Variaveis\Repeticoes\MesAno_TA_CA'
cmh = os.path.join(caminho, 'P21Mira') 
################################################Potreiro 20 Infestado############################################################# 
# load dataset
dataset1 = read_csv('e4_MesAno_TA_CA.csv', header=0, index_col=0, delimiter=';')
...
# load dataset
dataset3 = read_csv('entrada_Mira_MesAno_TA_CA.csv', header=0, index_col=0, delimiter=';')
values3 = dataset3.values
# integer encode direction
encoder = LabelEncoder()

dataset4 = read_csv('e4_Teste_MesAno_TA_CA.csv', header=0, index_col=0, delimiter=';')
...
if not os.path.exists('M_MIRA_MESANO_TA_CA.h5'):
	...
	#model.save('M_MIRA_MESANO_TA_CA.h5')
	#------------------
else:
	#------------------
	# load the network's architecture
	model = load_model('M_MIRA_MESANO_TA_CA.h5')
	history =  model
	#------------------
...
# load dataset
dataset5 = read_csv('entrada_TotalNaylor_MesAno_TA_CA.csv', header=0, index_col=0, delimiter=';')
values5 = dataset5.values
# integer encode direction
encoder = LabelEncoder()

dataset6 = read_csv('e4_Teste_MesAno_TA_CA.csv', header=0, index_col=0, delimiter=';')
...
if not os.path.exists('M_TOTALNAYLOR_MESANO_TA_CA.h5'):
	print('modelo inexistente... criando modelo...')
	# design network
	model = Sequential()
	#model.add(LSTM(30, input_shape=(train_X.shape[1], train_X.shape[2]),  kernel_initializer='normal', activation='tanh', return_sequences = True))#bom para 1 e 2
	...
	# save the network's architecture
	print('Salvando modelo...')
	#model.save('M_TOTALNAYLOR_MESANO.h5')
	#------------------
else:
	#------------------
	# load the network's architecture
	model = load_model('M_TOTALNAYLOR_MESANO_TA_CA.h5')
	history =  model
	#------------------
	print('Modelo encontrado e carregado')
...
pyplot.ylim(bottom=-4000)
pyplot.ylim(top=10000)
pyplot.ylabel('kg/ha')
fig.autofmt_xdate()
