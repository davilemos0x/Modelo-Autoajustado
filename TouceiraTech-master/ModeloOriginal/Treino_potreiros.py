...
caminho = r'C:\Users\Ânderson Fischoeder\Desktop\Tese\Modelos\ModeloOriginal\Repeticoes'
cmh = os.path.join(caminho, 'P21Mira') 
################################################Potreiro 20 Infestado############################################################# 
# load dataset
dataset1 = read_csv('e4.csv', header=0, index_col=0, delimiter=';')
...
dataset4 = read_csv('e4_Teste.csv', header=0, index_col=0, delimiter=';')
...
dataset6 = read_csv('e4_Teste.csv', header=0, index_col=0, delimiter=';')
...
if not os.path.exists('M_TOTAL_NAYLOR.h5'):
	print('modelo inexistente... criando modelo...')
	# design network
	model = Sequential()
	#model.add(LSTM(30, input_shape=(train_X.shape[1], train_X.shape[2]),  kernel_initializer='normal', activation='tanh', return_sequences = True))#bom para 1 e 2
	...
	# save the network's architecture
	print('Salvando modelo...')
	#model.save('M_TOTAL_NAYLOR.h5')
	#------------------
else:
	#------------------
	# load the network's architecture
	model = load_model('M_TOTAL_NAYLOR.h5')
	history =  model
	#------------------
	print('Modelo encontrado e carregado')
