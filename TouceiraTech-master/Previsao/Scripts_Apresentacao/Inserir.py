...
with open(r'C:\Users\Ânderson Fischoeder\Desktop\Tese\Modelos\Previsao\Nova pasta\e1_Treino.csv', 'r') as f:
    # Notice that we don't need the `csv` module.
    next(f) # Skip the header row.
    cur.copy_from(f, 'pastagens_ca', sep=';')
conn.commit()

with open(r'C:\Users\Ânderson Fischoeder\Desktop\Tese\Modelos\Previsao\Nova pasta\e3_Treino.csv', 'r') as f:
    # Notice that we don't need the `csv` module.
    next(f) # Skip the header row.
    cur.copy_from(f, 'pastagens_ca', sep=';')
conn.commit()